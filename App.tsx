...
const AppContent: React.FC = () => {
  ...
  const [consoleHistory, setConsoleHistory] = useState<LogEntry[]>([
      { type: 'info', content: `Conlang Studio OS v37.1 [Proportional-Engine Active]`, timestamp: '' },
  ...
  useEffect(() => {
    if (!isLoaded) return;
    const projectData: ProjectData = { version: "41.0", name: projectName, author: projectAuthor, description: projectDescription, lexicon, grammar, morphology, phonology, evolutionRules: rules, constraints, scriptConfig, lastModified: Date.now() };
  ...
  const getFullProjectData = (): ProjectData => ({ version: "41.0", name: projectName, author: projectAuthor, description: projectDescription, lexicon, grammar, morphology, phonology, evolutionRules: rules, constraints, scriptConfig, lastModified: Date.now() });
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-slate-900 text-slate-200 font-sans overflow-hidden transition-colors duration-200">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {isSidebarOpen ? (
            <div className={`flex-shrink-0 bg-slate-850 w-64 border-e border-slate-700 transition-all h-full ${isMobile ? 'absolute z-40 shadow-2xl' : 'relative'}`}>
            ...
            <div className="flex-shrink-0 bg-slate-850 w-12 border-e border-slate-700 flex flex-col items-center py-2 gap-1.5 z-20 overflow-y-auto no-scrollbar">
                <button onClick={() => setIsSidebarOpen(true)} className="text-slate-500 hover:text-white p-2 rounded hover:bg-slate-800 transition-colors mb-1"><PanelLeftOpen size={18} className={direction === 'rtl' ? 'rotate-180' : ''} /></button>
                <div className="w-6 h-px bg-slate-700 mb-1.5"></div>
                {navItems.map(item => ( <button key={item.id} onClick={() => setCurrentView(item.id as ViewState)} className={`p-2 rounded transition-colors ${currentView === item.id ? 'text-blue-400 bg-slate-800' : 'text-slate-500 hover:text-slate-200'}`} title={item.id}><item.icon size={18} /></button> ))}
            </div>
        )}
        <main className="flex-1 overflow-auto bg-slate-900 relative w-full" style={{ zoom: zoomLevel / 100 }}>{renderView()}</main>
      </div>
      <footer className="h-6 bg-slate-850 border-t border-slate-700 flex items-center px-4 text-xs text-slate-500 gap-4 shrink-0 z-50 relative">
