...
import { processCommandAI, repairLexicon } from '../services/geminiService';
...
const ConsoleConfig: React.FC<ConsoleConfigProps> = ({ 
      ...
      try {
          switch (cmd) {
              ...
              case 'FIX-NON-CANON':
                   ...
                   setLoadingAI(true); // Falsa carga para feedback visual
              ...
              case 'HELP':
                  ...
                  addLog('output', 'COMMANDAI [instrucción] - Modificación masiva del léxico mediante IA.');
                  addLog('output', 'CLEAR - Limpia el historial de la terminal.');
                  break;
              default: addLog('error', `Instrucción "${cmd}" no reconocida.`);
          }
  ...
  return (
    <div className="h-full flex flex-col bg-black font-mono text-sm relative">
        <div className="p-2 bg-zinc-900 border-b border-zinc-800 flex justify-between items-center text-xs text-zinc-500">
            <span className="flex items-center gap-2"><Terminal size={14} /> kernel_v4.6.0_stable</span>
        ...
        >
            {history.map((log, i) => (
                <div key={i} className={`flex flex-col ${log.type === 'error' ? 'text-red-500' : log.type === 'success' ? 'text-emerald-400' : log.type === 'command' ? 'text-white font-bold' : 'text-zinc-500'}`}>
