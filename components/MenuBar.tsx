...
const MenuBar: React.FC<MenuBarProps> = ({ 
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: t('menu.help'),
      items: [
        { label: t('menu.docs'), icon: HelpCircle, action: () => window.open('https://github.com', '_blank') },
  ...
  return (
    <header className="h-10 bg-slate-950 border-b border-slate-800 flex items-center px-2 select-none z-50">
      <input 
        type="file" 
        ref={fileInputRef} 
          ...
          >
            ...
            {activeMenu === menu.id && (
              <>
                ...
                <div className="absolute start-0 top-full w-56 bg-slate-900 border border-slate-700 shadow-xl rounded-b-md z-50 py-1">
                  {menu.items.map((item, idx) => {
                    if (item.type === 'separator') {
                      return <div key={idx} className="h-px bg-slate-700 my-1 mx-2" />;
                    }
                    const Icon = item.icon;
                    return (
                      <button
                        key={idx}
                        className="w-full text-left px-4 py-1.5 text-sm text-slate-300 hover:bg-blue-600 hover:text-white flex items-center justify-between group"
                        onClick={() => {
                          item.action?.();
                          setActiveMenu(null);
                        }}
                      >
                        <span className="flex items-center gap-2">
                          {Icon && <Icon size={14} className="text-slate-500 group-hover:text-white" />}
                          {item.label}
                        </span>
                        {item.shortcut && (
                          <span className="text-xs text-slate-500 group-hover:text-blue-100">{item.shortcut}</span>
                        )}
                      </button>
                    );
                  })}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="text-xs text-slate-500 mr-2">{t('menu.env')}: {getThemeLabel()}</div>
      </div>
    </header>
  );
};
