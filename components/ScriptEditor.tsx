    ...
    <div className="h-full flex flex-col bg-slate-950 overflow-hidden text-slate-200">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50 z-20">
            ...
            <div className="flex gap-4 items-center">
                ...
                <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded p-1">
                    <button onClick={() => setDrawMode('free')} className={`p-1.5 rounded ${drawMode === 'free' ? 'bg-purple-600' : 'hover:bg-slate-800'}`} title="Freehand"><Spline size={16} /></button>
                    <button onClick={() => setDrawMode('line')} className={`p-1.5 rounded ${drawMode === 'line' ? 'bg-purple-600' : 'hover:bg-slate-800'}`} title="Line"><Minus size={16} /></button>
                    <button onClick={() => setDrawMode('rect')} className={`p-1.5 rounded ${drawMode === 'rect' ? 'bg-purple-600' : 'hover:bg-slate-800'}`} title="Rectangle"><Square size={16} /></button>
                    <button onClick={() => setDrawMode('circle')} className={`p-1.5 rounded ${drawMode === 'circle' ? 'bg-purple-600' : 'hover:bg-slate-800'}`} title="Circle"><Circle size={16} /></button>
                </div>
                <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded px-2 py-1">
                    <button onClick={performUndo} disabled={undoStack.length === 0} className="p-1.5 hover:bg-slate-800 text-slate-500 disabled:opacity-20" title="Undo (Ctrl+Z)"><RotateCcw size={16} /></button>
                    <button onClick={performRedo} disabled={redoStack.length === 0} className="p-1.5 hover:bg-slate-800 text-slate-500 disabled:opacity-20" title="Redo (Ctrl+Y)"><RotateCw size={16} /></button>
                </div>
                <button onClick={saveGlyph} className={`px-4 py-2 rounded font-bold flex items-center gap-2 shadow-lg transition-all ${isDirty ? 'bg-purple-600 hover:bg-purple-700 scale-105' : 'bg-slate-800 text-slate-600'}`}>
        ...
        <div className="flex-1 flex overflow-hidden">
            <div className={`border-r border-slate-800 flex flex-col bg-slate-900/50 transition-all duration-300 ${sidebarCollapsed ? 'w-12' : 'w-72'}`}>
                <div className="p-3 border-b border-slate-800 flex justify-between items-center bg-slate-950">
                    {!sidebarCollapsed && <span className="text-xs font-bold text-slate-500 uppercase">Symbol Map</span>}
                    <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-1 hover:bg-slate-800 rounded text-slate-500">{sidebarCollapsed ? <ChevronRight size={16}/> : <ChevronLeft size={16}/>}</button>
                </div>
                
                {!sidebarCollapsed && (
                    <div className="p-2 border-b border-slate-800 bg-black/20 relative group">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-purple-500 transition-colors" size={14} />
                        <input 
                            value={sidebarSearch}
                            onChange={(e) => setSidebarSearch(e.target.value)}
                            className="w-full bg-slate-950 border border-slate-800 rounded pl-8 pr-2 py-1.5 text-xs text-slate-300 focus:border-purple-500 outline-none transition-all"
                ...
                <div className="flex-1 overflow-y-auto flex flex-wrap content-start p-2 gap-1 custom-scrollbar">
                    {Array.from({ length: 94 }, (_, i) => String.fromCharCode(i + 33))
                      ...
                      .map(char => {
                        ...
                        return (
                            <button key={char} onClick={() => setSelectedChar(char)} className={`w-12 h-12 rounded flex items-center justify-center font-mono font-bold text-sm transition-all relative ${selectedChar === char ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-800'}`}>
                                ...
                                {hasGlyph && <span className="absolute top-1 right-1 w-2 h-2 bg-emerald-400 rounded-full border-2 border-slate-950"></span>}
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 relative bg-[#020617] overflow-hidden" onWheel={handleCanvasWheel}>
                <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
                    <div className="bg-slate-900/90 backdrop-blur border border-slate-800 rounded-lg p-3 shadow-2xl flex flex-col gap-4 items-center min-w-[50px]">
                        <div className="flex flex-col gap-2 items-center">
                            <span className="text-[10px] text-slate-500 uppercase font-bold tracking-tighter">Size</span>
                            <div className="relative h-48 w-6 bg-slate-800 rounded-full overflow-hidden flex items-end">
                                <input 
                                    type="range" 
                                    min="1" 
                                    max="60" 
                                ...
                                />
                                <div className="absolute bottom-0 left-0 w-full bg-purple-600/20" style={{ height: `${(strokeWidth / 60) * 100}%` }}></div>
                            </div>
                            <span className="text-[10px] font-mono text-purple-400 font-bold">{strokeWidth}pt</span>
                        </div>
                        <label className="relative cursor-pointer p-2 hover:bg-slate-800 rounded flex justify-center border border-slate-700 group">
                            <Palette size={20} style={{ color: glyphColor }} className="group-hover:scale-110 transition-transform" />
                            <input type="color" value={glyphColor} onChange={(e) => setGlyphColor(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer" />
                        </label>
                    </div>
                </div>

                <div className="relative w-[400px] h-[400px] bg-slate-900/40 rounded-xl border border-slate-800/50 shadow-2xl overflow-hidden cursor-crosshair touch-none select-none transition-transform duration-200" style={{ transform: `scale(${canvasZoom})`, backgroundImage: showGrid ? 'radial-gradient(rgba(148,163,184,0.1) 1px, transparent 1px)' : 'none', backgroundSize: '20px 20px' }}>
                ...
                <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-slate-900/80 p-2 rounded border border-slate-800 text-[10px] font-mono text-slate-500 backdrop-blur">
            ...
            <div className="w-80 border-l border-slate-800 bg-slate-900/50 flex flex-col">
                <div className="p-3 border-b border-slate-800 flex items-center justify-between bg-slate-950">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Neural Layer Stack</span>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                    {[...strokes].reverse().map((s, revIdx) => {
                        ...
                        return (
                            <div key={s.id} onClick={() => setActiveLayerId(s.id)} className={`group border rounded-lg p-2 flex items-center justify-between transition-all cursor-pointer ${activeLayerId === s.id ? 'bg-purple-900/20 border-purple-500/50 shadow-inner' : 'bg-slate-950/40 border-slate-800 hover:border-slate-600'}`}>
                                <div className="flex items-center gap-3 overflow-hidden flex-1">
                                    <div className="flex flex-col gap-0.5">
                                        <button disabled={actualIdx === strokes.length - 1} onClick={(e) => { e.stopPropagation(); moveLayer(s.id, 'up'); }} className="p-0.5 text-slate-700 hover:text-purple-400 disabled:opacity-10"><ChevronUp size={12}/></button>
                                        <button disabled={actualIdx === 0} onClick={(e) => { e.stopPropagation(); moveLayer(s.id, 'down'); }} className="p-0.5 text-slate-700 hover:text-purple-400 disabled:opacity-10"><ChevronDown size={12}/></button>
                                    </div>
                                    <div className="w-10 h-10 bg-black rounded border border-zinc-800 flex items-center justify-center shrink-0 overflow-hidden relative">
                                        {s.type === 'image' ? (
                                    ...
                                    <div className="flex flex-col min-w-0 flex-1">
                                        {isEditing ? (
                                            ...
                                            <div className="flex items-center gap-2 group/label">
                                                <span className={`text-[10px] font-bold truncate ${activeLayerId === s.id ? 'text-purple-300' : 'text-slate-400'}`}>{s.label}</span>
                                                <button onClick={(e) => { e.stopPropagation(); setEditingLayerId(s.id); }} className="opacity-0 group-hover/label:opacity-100 text-slate-600 hover:text-purple-400"><Edit3 size={10}/></button>
                                            </div>
                                        )}
                                        <span className="text-[8px] text-slate-600 uppercase font-mono">{s.type} {s.type !== 'image' && `• ${s.strokeWidth}pt`}</span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-0.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                    <button onClick={(e) => { e.stopPropagation(); setStrokes(prev => prev.map(l => l.id === s.id ? {...l, locked: !l.locked} : l)); }} className={`p-1 rounded hover:bg-slate-800 ${s.locked ? 'text-amber-500' : 'text-slate-600'}`}>
                                        {s.locked ? <Lock size={12}/> : <Unlock size={12}/>}
                                    </button>
                                    <button onClick={(e) => { e.stopPropagation(); setStrokes(prev => prev.map(l => l.id === s.id ? {...l, visible: !l.visible} : l)); }} className={`p-1.5 rounded hover:bg-slate-800 ${s.visible ? 'text-slate-500' : 'text-blue-500'}`}>{s.visible ? <Eye size={12}/> : <EyeOff size={12}/>}</button>
                                    <button 
                                        ...
                                        className={`p-1.5 rounded hover:bg-red-900/20 ${strokes.length <= 1 ? 'text-slate-800 cursor-not-allowed' : 'text-slate-700 hover:text-red-500'}`}
                                    >
                                        <Trash2 size={12}/>
                                    </button>
                                </div>
                ...
                <div className="p-4 bg-slate-950 border-t border-slate-800 space-y-2">
                    <button onClick={() => fileInputRef.current?.click()} className="w-full py-2 bg-slate-900 border border-slate-800 text-slate-400 text-[10px] font-bold uppercase hover:bg-slate-800 hover:text-white transition-all flex items-center justify-center gap-2">
