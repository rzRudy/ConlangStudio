...
const Lexicon: React.FC<LexiconProps> = ({ 
  ...
  const renderEntryCard = (entry: LexiconEntry) => {
    ...
    return (
      <div key={entry.id} className={`bg-slate-900 border ${isInvalid ? 'border-red-900/30' : 'border-slate-800'} rounded-lg p-4 hover:border-slate-600 transition-colors group relative overflow-hidden`}>
        {entry.derivedFrom && <GitFork className={`absolute ${direction === 'rtl' ? '-left-4 -scale-x-100' : '-right-4'} -top-4 text-slate-800 opacity-20 w-32 h-32 rotate-12`} />}
        ...
        <div className="flex justify-between items-start relative z-10">
          <div className="w-full">
            <div className="flex items-baseline gap-3 mb-1">
              ...
              {isScriptMode ? (
                  <div className="flex flex-col">
                      ...
                      <span className="text-xs font-mono text-slate-500">{entry.word}</span>
                  </div>
              ) : (
                  <h3 className="text-2xl font-serif text-slate-100 font-bold">{entry.word}</h3>
              )}

              <span className="text-slate-400 font-mono text-sm bg-slate-800 px-2 py-0.5 rounded">/{entry.ipa}/</span>
              ...
              {isInvalid && (
                  <div className="group/badge relative">
                      ...
                      <div className="absolute left-0 bottom-full mb-2 hidden group-hover/badge:block bg-slate-900 text-slate-300 text-xs p-3 rounded-lg border border-slate-700 whitespace-nowrap z-50 shadow-xl">
                          <div className="font-bold text-slate-100 mb-1 flex items-center gap-2"><AlertTriangle size={12} className="text-red-400"/> {t('val.errors_title')}</div>
            ...
            <p className="text-slate-300 text-lg mb-3 mt-1">{entry.definition}</p>

            {(parent || descendants.length > 0 || entry.etymology) && (
              <div className="bg-slate-950/50 rounded-md p-3 border border-slate-800/50 space-y-2">
                {entry.etymology && <div className="text-slate-500 text-sm italic mb-1">"{entry.etymology}"</div>}
                <div className="flex flex-wrap items-center gap-y-2 text-sm">
                  {parent ? (
                    ...
                    <span className="text-xs uppercase font-bold tracking-widest border border-slate-700 px-1 rounded text-slate-600">Root</span>
                  )}
                  {descendants.length > 0 && (
                    <>
                      {parent && <span className="mx-2 text-slate-700">|</span>}
                      <div className="flex items-center gap-2 text-slate-400">
                        <GitFork size={12} />
                        <span>{t('lexicon.descendants')}:</span>
                        <div className="flex flex-wrap gap-2">
                          {descendants.map(desc => (
                            <button key={desc.id} onClick={() => jumpToWord(desc.word)} className="bg-slate-800 hover:bg-slate-700 text-blue-300 px-2 py-0.5 rounded text-xs transition-colors">{desc.word}</button>
                          ))}
                        </div>
                      </div>
                    </>
          ...
          <div className="flex gap-2 items-start">
             ...
             <button 
                ...
                className="p-2 text-slate-400 hover:bg-slate-800 hover:text-white rounded-md transition-colors relative border border-transparent hover:border-slate-700" 
             ...
             <button onClick={() => openEditModal(entry)} className="p-2 text-slate-400 hover:bg-slate-800 hover:text-white rounded-md transition-colors" title={t('lexicon.edit')}>
                 <Edit size={16} />
             </button>
      ...
      return (
          <div className="flex flex-col h-full bg-slate-950">
               <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex items-center gap-4">
                  <button onClick={() => setActiveTab('BROWSE')} className="text-slate-400 hover:text-white flex items-center gap-2 text-sm font-bold">
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950">
      <div className="flex flex-col border-b border-slate-800 bg-slate-900/50 sticky top-0 z-20 backdrop-blur-md">
          ...
          <div className="flex items-center gap-2 flex-1 max-w-2xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-700 text-slate-200 ps-10 pe-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-slate-600 transition-all shadow-sm"
              />
              {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute end-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-100"><X size={14} /></button>}
            </div>
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`p-2.5 rounded-lg border transition-colors flex items-center gap-2 ${showFilters || posFilter !== 'ALL' ? 'bg-blue-900/20 border-blue-600 text-blue-400' : 'bg-slate-900 border-slate-700 text-slate-400 hover:text-white hover:border-slate-500'}`}
            ...
            {totalConflictCount > 0 && (
                <button
                    ...
                    className={`p-2.5 rounded-lg border transition-all flex items-center gap-2 ${conflictMode !== 'HIDDEN' ? 'bg-red-950/20 border-red-500/30 text-red-400 hover:bg-red-900/30' : 'bg-slate-900 border-slate-700 text-slate-500 hover:text-slate-300'}`}
        ...
        {(showFilters || posFilter !== 'ALL') && (
            <div className="px-4 pb-4 animate-in slide-in-from-top-2 duration-200">
                <div className="bg-slate-950 border border-slate-800 rounded-lg p-3 flex flex-wrap items-center gap-4 shadow-inner">
                    <div className="flex items-center gap-2 border-r border-slate-800 pr-4 rtl:border-r-0 rtl:border-l rtl:pr-0 rtl:pl-4">
                        <Filter size={14} className="text-slate-500" />
                        <span className="text-xs font-bold text-slate-500 uppercase">{t('lexicon.pos')}:</span>
                        <select value={posFilter} onChange={(e) => setPosFilter(e.target.value)} className="bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded px-2 py-1 outline-none focus:border-blue-500">
                            <option value="ALL">{t('val.any_pos')}</option>
                            {POS_SUGGESTIONS.map(pos => <option key={pos} value={pos}>{getPosLabel(pos)}</option>)}
                        </select>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className="text-xs font-bold text-slate-500 uppercase">{t('lexicon.search_in')}:</span>
                        <label className="flex items-center gap-1.5 cursor-pointer text-xs text-slate-300 hover:text-white"><input type="checkbox" checked={searchFields.word} onChange={() => toggleField('word')} className="rounded border-slate-700 bg-slate-900 text-blue-600" />{t('lexicon.word')}</label>
                        <label className="flex items-center gap-1.5 cursor-pointer text-xs text-slate-300 hover:text-white"><input type="checkbox" checked={searchFields.definition} onChange={() => toggleField('definition')} className="rounded border-slate-700 bg-slate-900 text-blue-600" />{t('lexicon.definition')}</label>
                        <label className="flex items-center gap-1.5 cursor-pointer text-xs text-slate-300 hover:text-white"><input type="checkbox" checked={searchFields.etymology} onChange={() => toggleField('etymology')} className="rounded border-slate-700 bg-slate-900 text-blue-600" />{t('lexicon.etymology')}</label>
                         <label className="flex items-center gap-1.5 cursor-pointer text-xs text-slate-300 hover:text-white"><input type="checkbox" checked={searchFields.ipa} onChange={() => toggleField('ipa')} className="rounded border-slate-700 bg-slate-900 text-blue-600" />{t('lexicon.ipa')}</label>
                    </div>
                    <div className="ml-auto pl-4 border-l border-slate-800 rtl:border-l-0 rtl:border-r rtl:ml-0 rtl:mr-auto rtl:pr-0 rtl:pl-4">
                        <select value={conflictMode} onChange={(e) => setConflictMode(e.target.value as ConflictViewMode)} className="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-slate-400 outline-none focus:border-blue-500">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          {isSearchActive ? (
            <div className="space-y-4">
               {searchResults.length > 0 ? searchResults.map((entry) => renderEntryCard(entry)) : <div className="text-center py-20 text-slate-600"><Search size={48} className="mx-auto mb-4 opacity-50" /><p className="text-lg">{t('lexicon.no_matches')}</p><p className="text-sm">{t('lexicon.try_adjust')}</p></div>}
            </div>
          ) : (
            sortedKeys.length > 0 ? sortedKeys.map(letter => {
                ...
                return (
                    <div key={letter} className="space-y-4">
                        <div className={`flex items-center gap-4 sticky top-0 backdrop-blur-sm z-10 py-2 ${isConflictGroup ? 'bg-slate-950/95 text-red-400 border-b border-red-900/20' : 'bg-slate-950/90'}`}>
                            <h2 className={`text-4xl font-bold select-none flex items-center gap-3 ${isConflictGroup ? 'text-red-400 scale-90 origin-left rtl:origin-right' : 'text-slate-100'}`}>
                                {isConflictGroup && <AlertTriangle size={24} strokeWidth={2} />}
                                {letter}
                            </h2>
                            <div className={`h-px flex-1 ${isConflictGroup ? 'bg-red-900/20' : 'bg-slate-800'}`}></div>
                        </div>
            ...
            }) : <div className="text-center py-20 text-slate-600"><BookA size={48} className="mx-auto mb-4 opacity-50" /><p className="text-lg">{t('dashboard.empty_dict')}</p><p className="text-sm">{t('dashboard.create_first')}</p></div>
          )}
        </div>
      </div>
      ...
      {entryToDelete && dependentsWarning.length > 0 && (
          <div className="fixed inset-0 bg-red-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-[110]">
              <div className="bg-slate-900 border border-red-500/50 w-full max-w-lg rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                  <div className="p-6">
                      <div className="flex items-start gap-4">
                          <div className="p-3 bg-red-500/10 rounded-full shrink-0"><ShieldAlert className="text-red-500" size={32} /></div>
                          <div>
                              <h3 className="text-xl font-bold text-slate-50 mb-2">{t('integrity.title')}</h3>
                              <p className="text-slate-300 mb-4">{t('integrity.warning')}</p>
                              <div className="bg-slate-950 rounded border border-red-900/30 p-3 mb-4 max-h-40 overflow-y-auto">
                                  <div className="text-xs font-bold text-red-400 uppercase mb-2">Dependents:</div>
                                  <ul className="space-y-1">{dependentsWarning.map(d => <li key={d.id} className="text-sm text-slate-400 flex items-center gap-2"><ArrowRight className={direction === 'rtl' ? 'rotate-180' : ''} size={12} /> {d.word}</li>)}</ul>
                              </div>
                              <p className="text-xs text-slate-500 italic mb-6">{t('integrity.desc')}</p>
                              <div className="flex gap-3 justify-end">
                                  <button onClick={cancelDelete} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-100 rounded font-medium transition-colors">{t('integrity.action_cancel')}</button>
      ...
      {showSimpleDeleteConfirm && entryToDelete && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[110]">
              <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-xl shadow-2xl animate-in fade-in zoom-in duration-200">
                  <div className="p-6">
                      <div className="flex items-start gap-4">
                          <div className="p-3 bg-slate-800 rounded-full shrink-0"><HelpCircle className="text-slate-400" size={32} /></div>
                          <div>
                              <h3 className="text-xl font-bold text-slate-50 mb-2">{t('lexicon.delete_confirm_title')}</h3>
                              <p className="text-slate-300 mb-6">{t('lexicon.delete_confirm_desc')} <span className="font-bold text-slate-100">"{entryToDelete.word}"</span>? {t('lexicon.action_cannot_undo')}</p>
                              <div className="flex gap-3 justify-end">
                                  <button onClick={cancelDelete} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-100 rounded font-medium transition-colors">{t('common.cancel')}</button>
      ...
      {isModalOpen && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
            ...
            {(showIPAKeyboard || pinIPAKeyboard) && (
                <div className={`absolute z-[120] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 ${pinIPAKeyboard ? 'right-4 top-20 bottom-20 w-64' : 'bottom-10 left-1/2 -translate-x-1/2 w-[600px] h-[300px]'}`}>
                    <div className="flex items-center justify-between p-2 bg-slate-800 border-b border-slate-700 cursor-move">
                        ...
                        <div className="flex gap-1">
                            <button 
                                ...
                                className={`p-1 rounded hover:bg-slate-700 ${pinIPAKeyboard ? 'text-blue-400' : 'text-slate-500'}`}
                                title="Pin Panel"
                            >
                                {pinIPAKeyboard ? <PinOff size={12} /> : <Pin size={12} />}
                            </button>
                            <button onClick={() => { setShowIPAKeyboard(false); setPinIPAKeyboard(false); }} className="p-1 text-slate-500 hover:text-white">
                                <X size={12} />
                            </button>
                        </div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 bg-slate-950 custom-scrollbar">
                        {Object.entries(IPA_SYMBOLS).map(([category, symbols]) => (
                            <div key={category} className="mb-4">
                                <div className="text-[10px] font-bold text-slate-600 uppercase mb-1">{category}</div>
                                <div className="flex flex-wrap gap-1">
                                    {symbols.map(char => (
                                        <button
                                            key={char}
                                            onClick={() => handleIPAInsert(char)}
                                            className="w-8 h-8 flex items-center justify-center bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white rounded border border-slate-800 hover:border-slate-600 font-serif text-lg transition-colors"
                                        >
                                            {char}
                                        </button>
                                    ))}
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh] relative">
              ...
              <div className="p-6 space-y-4 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-400 uppercase">{t('lexicon.word')}</label>
                    <input autoFocus value={newWord} onChange={(e) => setNewWord(e.target.value)} className={`w-full bg-slate-950 border rounded p-2 text-slate-100 focus:outline-none focus:ring-1 ${validationErrors.length > 0 ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-slate-700 focus:border-blue-500 focus:ring-blue-500'}`} placeholder="e.g. kamra" />
                  </div>
                   <div className="space-y-1">
                    <label className="text-xs font-semibold text-slate-400 uppercase">{t('lexicon.ipa')}</label>
                    <div className="relative">
                        <input 
                            ...
                            className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-slate-100 font-mono focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" 
                            placeholder="e.g. ˈkam.ra" 
                        />
                        <button onClick={() => setShowIPAKeyboard(!showIPAKeyboard)} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-blue-400">
                            <Mic size={14} />
                        </button>
                    </div>
                  </div>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase">{t('lexicon.pos')}</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase">{t('lexicon.definition')}</label>
                  <textarea value={newDefinition} onChange={(e) => setNewDefinition(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-slate-100 h-24 focus:border-blue-500 focus:outline-none resize-none focus:ring-1 focus:ring-blue-500" placeholder="..." />
                </div>
                <div className="border-t border-slate-800 pt-4 mt-2">
                  <label className="text-xs font-bold text-slate-500 uppercase mb-3 block flex items-center gap-2"><GitFork size={12} /> {t('lexicon.etymology')}</label>
                  <div className="space-y-3">
                     <div className="space-y-1">
                      <label className="text-[10px] font-semibold text-slate-500 uppercase">{t('lexicon.derivedFrom')}</label>
                      <select value={newDerivedFrom} onChange={(e) => setNewDerivedFrom(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-slate-300 focus:border-blue-500 focus:outline-none">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-semibold text-slate-500 uppercase">{t('lexicon.etymology')}</label>
                      <input value={newEtymology} onChange={(e) => setNewEtymology(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-slate-100 focus:border-blue-500 focus:outline-none" placeholder="e.g. Borrowed from High Valyrian..." />
                    </div>
                  </div>
                </div>
              </div>
              <div className="px-6 py-4 bg-slate-950 border-t border-slate-800 flex justify-end gap-3 shrink-0">
                <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-slate-300 hover:text-white text-sm font-medium transition-colors">{t('lexicon.cancel')}</button>
                <button onClick={handleSaveEntry} disabled={validationErrors.length > 0} className={`px-4 py-2 text-white text-sm font-medium rounded-md shadow-lg transition-all active:scale-95 ${validationErrors.length > 0 ? 'bg-slate-700 cursor-not-allowed opacity-50' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-900/20'}`}>{t('lexicon.save')}</button>
              </div>
            </div>
          </div>
        )}
